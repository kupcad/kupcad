...
fn intersect2D(p1: [2]f64, p2: [2]f64, p3: [2]f64, p4: [2]f64) ?[2]f64 {
    ...
    if (t > 1e-5 and t < 1.0 - 1e-5 and u > 1e-5 and u < 1.0 - 1e-5) {
...
pub fn crossSectionBoolean(
    ...
    while (i < edges.items.len) {
        ...
        while (j < edges.items.len) {
            if (intersect2D(edges.items[i].start, edges.items[i].end, edges.items[j].start, edges.items[j].end)) |pt| {
    ...
    for (edges.items) |edge| {
        ...
        if (edge.is_a) {
            const in_b = booleans.isPointInPolygon2D(mid, poly_b);
            keep = switch (op) {
                .union_op => !in_b,
                .difference => !in_b,
                .intersection => in_b,
            ...
            const in_a = booleans.isPointInPolygon2D(mid, poly_a);
            keep = switch (op) {
                .union_op => !in_a,
                .difference => in_a,
                .intersection => in_a,
    ...
    if (kept_edges.items.len > 0) {
        ...
        while (true) {
            if (@abs(current_pt[0] - pts2d.items[0][0]) < 1e-4 and @abs(current_pt[1] - pts2d.items[0][1]) < 1e-4) {
                break;
            }
            ...
            for (kept_edges.items, 0..) |edge, idx| {
                if (!used[idx]) {
                    if (@abs(edge.start[0] - current_pt[0]) < 1e-4 and @abs(edge.start[1] - current_pt[1]) < 1e-4) {
                        current_pt = edge.end;
                        used[idx] = true;
                        found = true;
                        break;
