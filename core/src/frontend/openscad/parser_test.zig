...
test "OpenSCAD Parser: Module definition and CSG Tree" {
    ...
    const def_stmt = parser.b.tree.defStmt(mod_node);
    ...
    const body_block = parser.b.tree.block(body_node);
    ...
    const diff_block_payload = parser.b.tree.block(diff_block);
    ...
    const mod_call_block_payload = parser.b.tree.block(mod_call_block);
...
test "OpenSCAD Parser: For Loop and Range [start:step:end]" {
    ...
    const for_stmt = parser.b.tree.forStmt(node);
    ...
    const range_payload = parser.b.tree.range(range);

    const start = getNode(&parser, range_payload.start);
    try testing.expectEqual(@as(f64, 0.0), parser.b.tree.number(start));

    const step = getNode(&parser, range_payload.step);
    try testing.expectEqual(@as(f64, 2.0), parser.b.tree.number(step));

    const end = getNode(&parser, range_payload.end);
    try testing.expectEqual(@as(f64, 10.0), parser.b.tree.number(end));
}
...
test "OpenSCAD Parser: Function Definition & Includes" {
    ...
    const import_stmt = parser.b.tree.importStmt(inc_node);
    ...
    const def_stmt = parser.b.tree.defStmt(fn_node);
...
test "OpenSCAD Parser: Vector Comprehension" {
    ...
    const for_node = getNode(&parser, getNodes(&parser, parser.b.tree.nodeSpan(comp_node))[0]);

    try testing.expectEqual(ast.Tag.for_stmt, for_node.tag);
    const for_stmt = parser.b.tree.forStmt(for_node);
...
test "OpenSCAD Parser: Unbraced Operator Module Chaining" {
    ...
    const rotate_node = getNode(&parser, getNodes(&parser, parser.b.tree.block(top_block).stmts)[0]);
    ...
    const cube_node = getNode(&parser, getNodes(&parser, parser.b.tree.block(rotate_block).stmts)[0]);
...
test "OpenSCAD Parser: Scoped Block and Variable Shadowing" {
    ...
    const stmts = getNodes(&parser, parser.b.tree.block(program).stmts);
    ...
    try testing.expectEqual(@as(f64, 10.0), parser.b.tree.number(stmt0_val));

    // Statement 1: Standalone scope block
    const stmt1 = getNode(&parser, stmts[1]);
    const inner_stmts = getNodes(&parser, parser.b.tree.block(stmt1).stmts);
    ...
    try testing.expectEqual(@as(f64, 20.0), parser.b.tree.number(inner_stmt0_val));
...
test "OpenSCAD Parser: Special Variables and Children Calls" {
    ...
    try testing.expectEqualStrings("array", getStr(&parser, parser.b.tree.defStmt(mod).name));

    const mod_body = getNode(&parser, parser.b.tree.defStmt(mod).body);
    const for_node = getNode(&parser, getNodes(&parser, parser.b.tree.block(mod_body).stmts)[0]);
    ...
    const child_call = getNode(&parser, getNodes(&parser, parser.b.tree.block(trans_block).stmts)[0]);
...
test "OpenSCAD Parser: Assert and Echo Prefixes" {
    ...
    const assert_node = getNode(&parser, getNodes(&parser, parser.b.tree.block(echo_block).stmts)[0]);
    ...
    const cube_node = getNode(&parser, getNodes(&parser, parser.b.tree.block(assert_block).stmts)[0]);
    try testing.expectEqualStrings("cube", getStr(&parser, parser.b.tree.methodCall(cube_node).method_name));
}
...
test "OpenSCAD Parser: Let and If Expressions" {
    ...
    const let_stmts = getNodes(&parser, parser.b.tree.block(let_block).stmts);
...
test "OpenSCAD Parser: Local Quoted Includes and Unary Plus" {
    ...
    const inc_idx = try parser.parseStatement();
    const inc_node = getNode(&parser, inc_idx);
    try testing.expectEqualStrings("local_lib.scad", getStr(&parser, parser.b.tree.importStmt(inc_node).path));
...
test "OpenSCAD Parser: Expression-level Assert and Echo" {
    ...
    const assert_stmts = getNodes(&parser, parser.b.tree.block(assert_block).stmts);
    ...
    const echo_stmts = getNodes(&parser, parser.b.tree.block(echo_block).stmts);
...
test "OpenSCAD Parser: Array Literal Expansion (each)" {
    ...
    const elements = getNodes(&parser, parser.b.tree.nodeSpan(array_node));
    try testing.expectEqual(@as(usize, 3), elements.len);

    const el0 = getNode(&parser, elements[0]);
    try testing.expectEqual(@as(f64, 1.0), parser.b.tree.number(el0));
...
test "OpenSCAD Parser: Empty Arguments and Array Elements" {
    ...
    const arr = getNodes(&parser, parser.b.tree.nodeSpan(arg0_val));
    ...
    const cube_call = getNode(&parser, getNodes(&parser, parser.b.tree.block(block).stmts)[0]);
...
test "OpenSCAD Parser: Trailing Commas Leniency" {
    ...
    const let_block = getNode(&parser, getNodes(&parser, parser.b.tree.block(body).stmts)[0]);
    try testing.expectEqual(ast.Tag.block, let_block.tag);
...
test "OpenSCAD Parser: Mid-Expression Comments & Empty Statements" {
    ...
    const block_stmts = getNodes(&parser, parser.b.tree.block(body).stmts);
...
test "OpenSCAD Parser: C-Style Hexadecimal Constants" {
    ...
    try testing.expectEqual(@as(f64, 255.0), parser.b.tree.number(val));
}
...
test "OpenSCAD Parser: Leading-Dot Float Literals" {
    ...
    try testing.expectEqual(@as(f64, 0.5), parser.b.tree.number(left));

    const right = getNode(&parser, bin.right);
    try testing.expectEqual(@as(f64, 0.125), parser.b.tree.number(right));
}
...
test "OpenSCAD Parser: C-Style For Loops" {
    ...
    const stmts = getNodes(&parser, parser.b.tree.block(node).stmts);
    ...
    const while_stmt = parser.b.tree.whileStmt(while_node);
    ...
    const while_stmts = getNodes(&parser, parser.b.tree.block(while_body).stmts);
...
test "OpenSCAD Parser: Function Literals (Anonymous)" {
    ...
    const lambda = parser.b.tree.lambdaExpr(func_lit);
...
test "OpenSCAD Parser: Deeply Nested Let Expressions & Chained Expression Modifiers" {
    ...
    const let1_stmts = getNodes(&parser, parser.b.tree.block(let1).stmts);
    ...
    const let2_stmts = getNodes(&parser, parser.b.tree.block(let2).stmts);
...
test "OpenSCAD Parser: Diagnostics Line and Column Tracking" {
    ...
    // Validate exact coordinates using LineIndex (SoA token architecture)
    const line_index = try LineIndex.init(arena.allocator(), source);
    try testing.expectEqual(@as(u32, 2), line_index.getLine(diag.loc.offset));
    try testing.expectEqual(@as(u32, 18), line_index.getUtf8Column(diag.loc.offset));
}
